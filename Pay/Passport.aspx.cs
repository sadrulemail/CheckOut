    ...
    {  
        ...
        try
        {
            ...
            if (txtExpDate.Text != "")
            {
                ...
                int Year = GetDifferenceInYears(date) + 1;
                Pass_Fine = Year * 300;

                if (Year > 10)
                {
                    CommonControl1.ClientMsg("More than 10 years old passport can not be renewed.", txtCaptcha);
                    return;
                }
            }
            

            Pass_amount = (Pass_amount + Pass_Fine);
            Pass_amount = Pass_amount + (Pass_amount * Vat_Passport / 100);
