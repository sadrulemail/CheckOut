...
public partial class Checkout_Payment : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack) GetPaymentMarchentName();
        ...
        if (!IsPostBack)
        {
            ...
            lblTBMM_PaymentAmount.Text = lblVisa_PaymentAmount.Text = lblVM_PaymentAmount.Text = string.Format("{0:N2}", Amount);
            LoadServiceCharges();
        }
        ...
        lblVM_Total.Text = string.Format("{0:N2}", Amount + double.Parse(lblVM_Charge.Text));
    ...
    private void LoadServiceCharges()
    {
        ...
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            Query = "Select dbo.f_Checkout_Service_Charge ('" + Amount + "','" + hidMerchantID.Value + "','VM','" + ddlEMI.SelectedValue + "') as Charge";
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.CommandText = Query;
                cmd.Connection = conn;
                using (SqlDataReader sdr = cmd.ExecuteReader())
                {
                    while (sdr.Read())
                    {
                        lblVM_Charge.Text = string.Format("{0:N2}", sdr["Charge"]);
                    }
                }
            }
    ...
    private void GetPaymentMarchentName()
    {
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                ServiceCharge_vm = (bool)dt.Rows[0]["PayWithCharge_VM"];
    ...
    protected void lnk_Command(object sender, CommandEventArgs e)
    {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                using (SqlDataReader sdr = cmd.ExecuteReader())
                {
                    while (sdr.Read())
                    {
                        ...
                        TerminalID = string.Format("{0}", sdr["TerminalID"]);
                    }
                }
            }
        }
        ...
        else if (PaymentType == "VM")
        {
            TotalCost = double.Parse(lblVM_Total.Text);
        }
        ...
        else if(PaymentType == "VM")
        {
            try
            {
                string ReqXmlDoc = getReqXmlDocVM(oRow.Amount, "050", hidMerchantID.Value.Trim() + " " + RefID, Merchant, RefID, TerminalID);

                string postbackUrl = Common.getValueOfKey("VM_PaymentURL");
            ...
            {
                PanelError.Visible = true;
                PanelError.Controls.Add(new LiteralControl("Error Accessing VISA/Master Card Server, please try later." + "<br />" + ee.Message));
                LoadServiceCharges();
                return;
            }
        }
    ...
    private string getReqXmlDocVM(double _Amount, string _Currency, string _Description, string _MerchantID, string _M_Trans_ID, string _M_Terminal_ID)
    {
        XmlDocument doc = new XmlDocument();
        ...
        XmlNode PGNode = doc.CreateElement("PG");
        doc.AppendChild(PGNode);

        XmlNode MerchantChild = doc.CreateElement("Merchant_ID");
        MerchantChild.InnerText = _MerchantID;
        PGNode.AppendChild(MerchantChild);

        XmlNode AmountChild = doc.CreateElement("Amount");
        AmountChild.InnerText = string.Format("{0:N2}", _Amount).Replace(",", "");
        PGNode.AppendChild(AmountChild);

        XmlNode CurreccyChild = doc.CreateElement("Currency_ID");
        CurreccyChild.InnerText = _Currency;
        PGNode.AppendChild(CurreccyChild);

        XmlNode DescriptionChild = doc.CreateElement("Trans_Desc");
        DescriptionChild.InnerText = _Description;
        PGNode.AppendChild(DescriptionChild);

        XmlNode ApproveURLChild = doc.CreateElement("M_Approval_URL");
        ApproveURLChild.InnerText = Common.getValueOfKey("VM_ApproveURL");
        PGNode.AppendChild(ApproveURLChild);

        XmlNode CancelledURLChild = doc.CreateElement("M_Cancel_URL");
        CancelledURLChild.InnerText = Common.getValueOfKey("VM_CancelURL");
        PGNode.AppendChild(CancelledURLChild);

        XmlNode DeclineURLChild = doc.CreateElement("M_Decline_URL");
        DeclineURLChild.InnerText = Common.getValueOfKey("VM_DeclineURL");
        PGNode.AppendChild(DeclineURLChild);

        XmlNode M_Trans_IDChild = doc.CreateElement("M_Trans_ID");
        M_Trans_IDChild.InnerText = _M_Trans_ID;
        PGNode.AppendChild(M_Trans_IDChild);

        XmlNode M_Terminal_IDChild = doc.CreateElement("M_Terminal_ID");
        M_Terminal_IDChild.InnerText = _M_Terminal_ID;
        PGNode.AppendChild(M_Terminal_IDChild);

        XmlNode Trans_DateChild = doc.CreateElement("Trans_Date");
        Trans_DateChild.InnerText = string.Format("{0:yyyy-MM-dd HH:mm:ss}", DateTime.Now);
        PGNode.AppendChild(Trans_DateChild);
