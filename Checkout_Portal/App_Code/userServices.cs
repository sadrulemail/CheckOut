...
public class userServices : System.Web.Services.WebService
{
    ...
    public string getUserInfo(string contextKey)
    {
        ...
        while (oReader.Read())
        {
            ...
            Retval = string.Format("{0}<tr><td><a href='../Profile.aspx?ID={2}' title='view profile' target='_blank'><img src='" + getValueOfKey("Prefix") + "EmpImage.ashx?EMPID={2}&W=120&H=120&imgid={3}' width='60'></a></td><td nowrap=nowrap>{1}</td></tr>", Retval, oReader["Emp"], contextKey, oReader["ImageIdentifier"]);
    ...
    public string getBranchInfo(string contextKey)
    {
        ...
        oCommand.CommandType = CommandType.StoredProcedure;
        oCommand.Parameters.Add("@BranchID", SqlDbType.VarChar).Value = contextKey;
