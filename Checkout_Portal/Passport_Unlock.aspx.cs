...
public partial class Passport_Unlock : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (string.Format("{0}", Request.QueryString["refid"]) == "")
            {
                ...
                PanelPassportUnlock.Visible = false;
    ...
    {
        Response.Redirect("Passport_Unlock.aspx?refid=" + txtFilter.Text.Trim().ToUpper(), true);
        return;
    ...
    {
        string Msg = "Unlocked not Completed.";

        using (SqlConnection conn = new SqlConnection())
        {
            string Query = "s_Payments_Passport_Unlock";
            conn.ConnectionString = ConfigurationManager.ConnectionStrings["PaymentsDBConnectionString"].ConnectionString;
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                cmd.Parameters.Add("@RefID", System.Data.SqlDbType.VarChar).Value = string.Format("{0}", Request.QueryString["refid"]); 
                cmd.Parameters.Add("@EmpID", System.Data.SqlDbType.VarChar).Value = Session["EMPID"].ToString();
              
                cmd.Parameters.Add("@Reason", System.Data.SqlDbType.VarChar).Value = txtResion.Text.Trim();
